...
{{ image(path="/images/build-own-store-with-usesyncexternalstore/GlobalStore.png", alt="gloabl store") }}
...
需要注意的是，`useSyncExternalStore` 会立即调用 `subscribe` 和 `getSnapshot` ，这就导致了我们不能在这些方法中使用 `this.store` ，此时的 `this` 还未准备好。
...
{{ image(path="/images/build-own-store-with-usesyncexternalstore/ReduxDataFlowDiagram.gif", alt="Redux Data Flow Diagram") }}
